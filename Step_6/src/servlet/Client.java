...
VERSION 6.0
 ...
 1. A servlet which uses the data from a text file to build and customize an auto model
 * using web server.
 2. There is no need to implement multi threading separately in the servlet as Tomcat
 * implements multi threading automatically.
 * 
 3. As soon as the servlet is started, a file is read and the data is stored 
   as an auto object
  4. A web application's context path is the directory that contains the web
    	 *  application's WEB-INF directory. It is different from a class path and
    	 * has to be explicitly passed on to read or write to files.
    	 *getRealPath() would get the actual path for the servlet's home directory.
    	 
  5. 2 different ways are used to send multiple values to jsp
    	 *1) use the hidden input type with name and value parameters
    	 *2) use the value parameter with "," separating the values
    	 *
   6. when the button of type submit is pressed, the form is submitted to the jsp file
		 and the jsp file is displayed
7.post method is used as it is secure and since large amounts of data is being sent
		to the server
 8. store the values that has to be passed to JSP in a hidden input type object and pass it to jsp
 9. write methods in doGet and call doGet from doPost if you want both the methods to do
		//the same things - makes debugging easier
10. get the option set name and call method to print the options
			//for each option set
11. input of hidden type is used to pass the option set name to jsp
12. To get the selected option name, the name of select tag is used to
			 * get the parameter and the value corresponds to the option tag value
13. get the option names and the prices from the auto object and create a drop down of all options
14. To pass 2 values to the jsp, the value field of option is used,
		the 2 values to be passed are separated by "," passed to the jsp where
		the 2 values are split and passed to the correct variables
15. the value stores the option name and price with a comma separating the 2
	the value is then separated in the jsp file to the correct variables
 ...
 * A servlet which uses the data from a text file to build and customize an auto model
 * using web server.
 * There is no need to implement multi threading separately in the servlet as Tomcat
 * implements multi threading automatically.
 * 
 */
@WebServlet("/Client")
public class Client extends HttpServlet {
	...
	//Automotive object to store the properties from the file
	private Automobile auto1 = new Automobile();
    ...
    public void init()
    {
    	//As soon as the servlet is started, a file is read and the data is stored 
    	//as an auto object
    	/*
    	 * A web application's context path is the directory that contains the web
    	 *  application's WEB-INF directory. It is different from a class path and
    	 * has to be explicitly passed on to read or write to files.
    	 *getRealPath() would get the actual path for the servlet's home directory.
    	 
    	 *2 different ways are used to send multiple values to jsp
    	 *1) use the hidden input type with name and value parameters
    	 *2) use the value parameter with "," separating the values
    	 */
    	//reads the data file and stores it in auto object
    	String path = getServletContext().getRealPath("/");
    	auto1 = file1.readData(path + "/WEB-INF/Ford_focus.txt", auto1);
    	/* for other models*/
    	//auto1 = file1.readData(path + "/WEB-INF/Tesla_S.txt", auto1);
    	//auto1 = file1.readData(path + "/WEB-INF/Toyota_Prius.txt", auto1);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException{
		...
		//HTML tags to print a form with tables
		...
		out.println("</head>");
		out.println("<body>");
		//when the button of type submit is pressed, the form is submitted to the jsp file
		// and the jsp file is displayed
		//post method is used as it is secure and since large amounts of data is being sent
		//to the server
		out.println("<form action=\"Options.jsp\" method=\"post\">");
		out.println("<h3>Basic car choice</h3>");
		out.println("<table border=\"1\" style=\"width:100%\">");
		out.println("<tr>");
		out.println("<td>Make/Model:</td>");
		//store the values in a hidden input type object and pass it to jsp
		out.println("<input type =\"hidden\" name = \"make\"value =\""+auto1.getName()+"\">");
		out.println("<input type =\"hidden\" name = \"price\"value =\""+auto1.getPrice()+"\">");
		out.println("<td> "+auto1.getName()+"</td>");
		out.println("</tr>");
		printOptionSets(out);
		out.println("<tr>");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException{
		...
		//write methods in doGet and call doGet from doPost if you want both the methods to do
		//the same things - makes debugging easier
...
/*
 * Helper methods to print the option sets and options by passing the open PrintWriter	
 */
	protected void printOptionSets(PrintWriter pw)
	{
		//loop to print all the option sets
		for(int i = 0;i < auto1.getOpset().length;i++)
		{
			//get the option set name and call method to print the options
			//for each option set
			pw.println("<tr>");
			pw.println("<td>"+auto1.getOpsetNameByIndex(i)+"</td>");
			/*
			 * input of hidden type is used to pass the option set name to jsp
			 */
			pw.println(" <input type=\"hidden\" name =\"optionset"+i+"\" value=\""+auto1.getOpsetNameByIndex(i)+"\">");
			pw.println("<td>");
			/*
			 * To get the selected option name, the name of select tag is used to
			 * get the parameter and the value corresponds to the option tag value
			 */
			pw.println("<select name=\"optionname"+i+"\">");			
			printOption(pw,auto1.getOpsetByIndex(i),i);
			pw.println("</select>");
			pw.println("</td>");
			pw.println("</tr>");
		}
	...
	protected void printOption(PrintWriter pw, OptionSet o1, int i)
	{
		//get the option names and the prices from the auto object
		String[] allOpt = auto1.getOptNamesOfOpSet(o1);
		float[] allOptPrice = auto1.getOptPricesOfOpSet(o1);
		//create a drop down of all options
		for(int j=0;j<allOpt.length;j++)
		{	/*
		To pass 2 values to the jsp, the value field of option is used,
		the 2 values to be passed are separated by "," passed to the jsp where
		the 2 values are split and passed to the correct variables
		*/
			//the value stores the option name and price with a comma separating the 2
			//the value is then separated in the jsp file to the correct variables
			pw.println("<option value =\""+allOpt[j]+","+allOptPrice[j]+"\">");
			pw.println(allOpt[j]);
			pw.println("</option>");	
		}
	}
